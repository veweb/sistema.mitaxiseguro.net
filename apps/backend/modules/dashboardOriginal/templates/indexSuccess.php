 ...
 $(document).ready(function(){
	...
	$("#btns_submit").click(function() {
      ...
      fancyConfirm('Esta seguro enviar el servicio?', function() {
                ...
                $.ajax({
			    type:'post',
			    url:'<?php echo url_for('dashboard/mensaje'); ?>',
			    data:dataEm,
			    beforeSend:function(){
...
<form name="datos" id="datos" action="<?php echo url_for('dashboard/index'); ?>" method="POST">
				<div id="bodytop">
				...
				<select id="trafico" name="trafico">
					...
					<?php if($tarifa->getId() == 2): ?>
					      <option value="<?php echo $tarifa->getId(); ?>" selected=selected><?php echo $tarifa->getName();  ?></option>
				    <?php else: ?>
				...
				<input type="text" name="puntoA" class="puntoB" id="puntoA" value="<?php echo $refA; ?>" style="background-color=#78B70B">  
				<input type="text" name="puntoB" class="puntoA" id="puntoB" value="<?php echo $puntoB; ?>">  
...
<div id="module">
  ...
  callAjax2();
				...
				<script type="text/javascript">
                  $(document).ready(function(){
				    ...
				    var end = $("#puntoB").val();//'14.5970354, -90.50452059999998';
				       var trafico = $("#trafico").val();
				    ...
				    directionsService.route(request, function(response, status) {
				      if (status == google.maps.DirectionsStatus.OK) {
				        ...
				        */
				        var distancia=parseFloat(myRoute.legs[0].distance.text.replace(",",".")).toFixed(2); 
 ...
 google.maps.event.addListener(autocompleteB, 'place_changed', function() {
  ...
  //setupClickListener('changetype-geocode', ['geocode']);
				...
				<form name="mensaje" id="form_account" action="<?php echo url_for('dashboard/mensaje'); ?>" method="POST"> 
				<div id="unidades">
					<h3 class="tabtit">UNIDADES <?php echo "ECONTRADAS: ".count($markerUnidades);  ?></h3>
					<table>
						<tr>
				...
				<div id="cliente">
					...
					<div id="clientecont">
						<div id="clientenom">
							...
							<input type="text" name="cliente[]" id="clienten"  value="<?php echo $cname; ?>"/>
						</div>
						<div id="clientedir">
							<h4>Direccion Origen:</h4>
							<input type="text" name="cliente[]" id="clienten" value="<?php echo $direccion; ?>" />
						</div>
						<div id="clientedira">
							<h4>Direccion Destino:</h4>
							<input type="text" name="cliente[]" id="clienten" value="<?php echo $direccionB; ?>" />
						</div>
						<div id="clientetel">
							<h4>Telefono:</h4>
							<input type="text" name="cliente[]" id="clienten" value="<?php echo $cphone; ?>"  />
						</div>
						<div id="clientecom">
							<h4>Comentarios:</h4>
							<input type="text" name="cliente[]" id="clienten" value="<?php echo $descripcion; ?>" />
						</div>
					</div>
				</div>
